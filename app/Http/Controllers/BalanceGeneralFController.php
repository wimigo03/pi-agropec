...
use App\Exportar\BalanceGeneralFExcel;
use DB;
use PDF;

class BalanceGeneralFController extends Controller
{
    ...
    public function excel(Request $request)
    {
        ...
        try {
            ...
            ini_set('max_execution_time','-1');
                ...
                $total = $estado_resultado['total'];
                $totales = $estado_resultado['totales'];
            return Excel::download(new BalanceGeneralFExcel($empresa,$ingresos,$costos,$gastos,$nroMaxColumna,$total,$totales,$fecha_f),'Balance_General.xlsx');
        } catch (\Throwable $th) {
    ...
    public function pdf(Request $request)
    {
        ...
        try {
            ...
            ini_set('max_execution_time','-1');
                ...
                $total = $estado_resultado['total'];
                $totales = $estado_resultado['totales'];
                $pdf = PDF::loadView('balance_general_f.pdf',compact(['empresa','ingresos','costos','gastos','nroMaxColumna','total','totales','fecha_f']));
